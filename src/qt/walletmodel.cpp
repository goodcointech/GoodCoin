...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered goodcoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 GDC.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal goodcoin:URI (goodcoin:XyZ...?message=example)
            {
...
static void NotifyzGDCReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzGDCReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzGDCReset.connect(boost::bind(NotifyzGDCReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzGDCReset.disconnect(boost::bind(NotifyzGDCReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zgdcTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
