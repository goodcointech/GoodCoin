...
CzGDCWallet::CzGDCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zgdc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGDCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGDCSeed_deprecated()) {
                    LogPrintf("%s: Updated zGDC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zgdc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zgdc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzGDCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGDCCount(nCountLastUsed);
    else if (!walletdb.ReadZGDCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzGDCWallet::Lock()
{
    seedMaster = 0;
}

void CzGDCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzGDCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGDC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzGDCWallet::LoadMintPoolFromDB()
{
...
void CzGDCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzGDCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzGDCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zgdcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzGDCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGDC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zgdcTracker which also adds to database
    pwalletMain->zgdcTracker->Add(dMint, true);
...
void CzGDCWallet::SeedToZGDC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzGDCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzGDCWallet::GenerateDeterministicZGDC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzGDCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzGDCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
